function [WB_SDMO] = extract_JerkMeanRatio_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_JerkMeanRatio_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).JerkRatio_SigComplete = Ratio of jerk (1st derivative of acceleration) of complete signal (walking bout in the given imu data),i.e. jerk on each component / jerk on resultant
% WB_SDMO(i).JerkRatio_AvPerSteps = average Ratio of jerk (1st derivative of acceleration) of all steps within the WB (walking bout in the given imu data), i.e. jerk on each component / jerk on resultant
% WB_SDMO(i).JerkRatio_AvPerStrides = average Ratio of jerk (1st derivative of acceleration) of all stride within the WB (walking bout in the given imu data), i.e. jerk on each component / jerk on resultant
...
for WB_index = 1:WB_Number                                                  % loop over each WB
    ...
    % Initialization
    clear JerkRatio_SigComplete
    clear JerkRatio_AvPerSteps
    clear JerkRatio_AvPerStrides
    for iChannel = 1:size(NameChannels)-1
        JerkRatio_SigComplete.(NameChannels{iChannel}) = [];
        JerkRatio_AvPerSteps.(NameChannels{iChannel}) = [];
        JerkRatio_AvPerStrides.(NameChannels{iChannel}) = [];
    end
    ...
    for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
        ...
        % Calculation of Mean Ratio of Jerk - Acc
        if StepsNumber > Settings.MinNumEvents
            for iChannel = 1:size(Acc_Detr_LPFilt,2)-1
                JerkRatio_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = mean((diff(Acc_Detr_LPFilt(:,iChannel))*fs)./(diff(Acc_Detr_LPFilt(:,4))*fs));% Jerk is the 1st derivative of acceleration, then, we calcuate the ratio: jerk on each component / jerk on resultant
                for iIC = 1:size(IC,2)-1
                    JerkRatio_PerSteps(iIC,iChannel) = mean((diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs)./(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),4))*fs));
                    if iIC < size(IC,2)-1
                        JerkRatio_PerStrides(iIC,iChannel) = mean((diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs)./(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),4))*fs));
                    end
                end
                JerkRatio_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(JerkRatio_PerSteps(:,iChannel));
                JerkRatio_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(JerkRatio_PerStrides(:,iChannel));
            end
        else
            for iChannel = 1:size(Acc_Detr_LPFilt,2)-1
                JerkRatio_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                JerkRatio_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                JerkRatio_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nan;
            end
        end
    end
    ...
    % Outputs
    for iChannel = 1:size(Acc_Detr_LPFilt,2)-1
        WB_SDMO(WB_index).JerkMeanRatio_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(JerkRatio_SigComplete,(NameChannels{iChannel})));
        WB_SDMO(WB_index).JerkMeanRatio_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(JerkRatio_AvPerSteps,(NameChannels{iChannel})));
        WB_SDMO(WB_index).JerkMeanRatio_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(JerkRatio_AvPerStrides,(NameChannels{iChannel})));
    end    
end
