function [WB_SDMO] = extract_JerkMin_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_JerkMin_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).JerkMin_SigComplete = Min of jerk (1st derivative of acceleration) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).JerkMin_AvPerSteps = average Min of jerk (1st derivative of acceleration) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).JerkMin_AvPerStrides = average Min of jerk (1st derivative of acceleration) of all stride within the WB (walking bout in the given imu data).
...
% Initialization
clear JerkMin_SigComplete
clear JerkMin_AvPerSteps
clear JerkMin_AvPerStrides
for iChannel = 1:size(NameChannels,2)
    JerkMin_SigComplete.(NameChannels{iChannel}) = [];
    JerkMin_AvPerSteps.(NameChannels{iChannel}) = [];
    JerkMin_AvPerStrides.(NameChannels{iChannel}) = [];
end
...
% Calculation of Min of Jerk - Acc
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkMin_SigComplete.(NameChannels{iChannel}) = min(diff(Acc_Detr_LPFilt(:,iChannel))*fs); % Jerk is the 1st derivative of acceleration, over it, we obtain the min
        for iIC = 1:size(IC,2)-1
            JerkMin_PerSteps(iIC,iChannel) = min(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
            if iIC < size(IC,2)-1
                JerkMin_PerStrides(iIC,iChannel) = min(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
            end
        end
        JerkMin_AvPerSteps.(NameChannels{iChannel}) = nanmean(JerkMin_PerSteps(:,iChannel));
        JerkMin_AvPerStrides.(NameChannels{iChannel}) = nanmean(JerkMin_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkMin_SigComplete.(NameChannels{iChannel}) = nan;
        JerkMin_AvPerSteps.(NameChannels{iChannel}) = nan;
        JerkMin_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.JerkMin_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(JerkMin_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.JerkMin_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(JerkMin_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.JerkMin_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(JerkMin_AvPerStrides,(NameChannels{iChannel})));
end    
%end
