function [WB_SDMO] = extract_correlation_AccGyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_correlation_AccGyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Correlation of Acc with Gyro data
% No Reference
%
%% Input
...
%% Output
% WB_SDMO(i).CorrAccGyr_SigComplete = correlation between Acc and Gyr data for the complete signal (walking bout in the given imu data).
% WB_SDMO(i).CorrAccGyr_AvPerSteps = correlation between Acc and Gyr data for all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).CorrAccGyr_AvPerStrides = correlation between Acc and Gyr data for all strides within the WB (walking bout in the given imu data).
...
NameChannels = {'VT_Yaw','ML_Pitch','AP_Roll','Combined2'};

% Initialization
clear CorrAccGyr_SigComplete
clear CorrAccGyr_AvPerSteps
clear CorrAccGyr_AvPerStrides
for iChannel = 1:size(NameChannels,2)
    CorrAccGyr_SigComplete.(NameChannels{iChannel}) = [];
    CorrAccGyr_AvPerSteps.(NameChannels{iChannel}) = [];
    CorrAccGyr_AvPerStrides.(NameChannels{iChannel}) = [];
end
%for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
    
% ACC: Definition of signal: start-end
...
% GYRO: Definition of signal: start-end
...
% Calculation of RMS of Angular Acceleration - Gyro
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        GyroData = Gyr_Detr_LPFilt(:,iChannel);
        AccData = Acc_Detr_LPFilt(:,iChannel);                
        Corr_SigComplete = corrcoef([GyroData AccData]);
        CorrAccGyr_SigComplete.(NameChannels{iChannel}) = (Corr_SigComplete(2,1)).^2;               
        for iIC = 1:size(IC,2)-1
            GyroData_ = Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel);
            AccData_ = Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel);
            Corr_PerSteps = corrcoef([GyroData_ AccData_]);                    
            CorrAccGyr_PerSteps(iIC,iChannel) = (Corr_PerSteps(2,1)).^2;
            if iIC < size(IC,2)-1
                GyroData__ = Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel);
                AccData__ = Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel);
                Corr_PerStrides = corrcoef([GyroData__ AccData__]);
                CorrAccGyr_PerStrides(iIC,iChannel) = (Corr_PerStrides(2,1)).^2;
            end
        end
        CorrAccGyr_AvPerSteps.(NameChannels{iChannel}) = nanmean(CorrAccGyr_PerSteps(:,iChannel));
        CorrAccGyr_AvPerStrides.(NameChannels{iChannel}) = nanmean(CorrAccGyr_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        CorrAccGyr_SigComplete.(NameChannels{iChannel}) = nan;
        CorrAccGyr_AvPerSteps.(NameChannels{iChannel}) = nan;
        CorrAccGyr_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Gyr_Detr_LPFilt,2)
    WB_SDMO.CorrAccGyr_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(CorrAccGyr_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.CorrAccGyr_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(CorrAccGyr_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.CorrAccGyr_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(CorrAccGyr_AvPerStrides,(NameChannels{iChannel})));
end    
%end
