function [WB_SDMO] = extract_PhaseFeatures_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_PhaseFeatures_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of phase plot features - ANGULAR VELOCITY / GYROSCOPE
...
%% Output
% WB_SDMO(i).PhaseFeatures_Gyr = Phase plot features obtained from angular velocity data, for each signal-axis (the original script was based primarly on VT Acceleration)
% Several adaptations on the original code were performed
...
for WB_index = 1:WB_Number                                                  % loop over each WB
    ...
    NameChannels = {'Yaw','Pitch','Roll','Res'};
    ...
    for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
       ...
       % Detrend & filter signals
        ...
        % Calculation of phase features calculation - Gyro
        if  StepsNumber > Settings.MinNumStrides_Complex
            TimeVect = ones(size(Gyr_Detr_LPFilt,1),1);
            AccSigInput = [TimeVect,Gyr_Detr_LPFilt];
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                ...
                PhaseFeatures(StraightWalk_Index).(NameChannels{iChannel}) = Phase_analysis_Gyro(AccSigInput, NameChannelsPhase{iChannel}, fs, Settings.MinNumStrides_Complex, PlotSettings.PhaseFeatures, PhaseFeaturesPlotDelay);
            end
    ...
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        for iPhase = 1:size(VariablesPhaseNames,1)
            ...
            WB_SDMO(WB_index).PhaseFeatures_Gyr.(NameChannels{iChannel}).(VariablesPhaseNames{iPhase}) = nanmean(DataStraightWalk);
