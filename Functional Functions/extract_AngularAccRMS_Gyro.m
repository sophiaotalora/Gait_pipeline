function [WB_SDMO] = extract_AngularAccRMS_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_AngularAccRMS_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).AngAccRMS_SigComplete = RMS of angular acceleration (1st derivative of angular velocity) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).AngAccRMS_AvPerSteps = average RMS of angular acceleration (1st derivative of angular velocity) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).AngAccRMS_AvPerStrides = average RMS of angular acceleration (1st derivative of angular velocity) of all stride within the WB (walking bout in the given imu data).
...
for WB_index = 1:WB_Number                                                  % loop over each WB
    ...
    % Initialization
    clear AngAccRMS_AvPerSteps
    clear AngAccRMS_AvPerSteps
    clear AngAccRMS_AvPerStrides
    for iChannel = 1:size(NameChannels,2)
        AngAccRMS_SigComplete.(NameChannels{iChannel}) = [];
        AngAccRMS_AvPerSteps.(NameChannels{iChannel}) = [];
        AngAccRMS_AvPerStrides.(NameChannels{iChannel}) = [];
    end
    for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
        ...
        if StepsNumber > Settings.MinNumEvents
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccRMS_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = rms(diff(Gyr_Detr_LPFilt(:,iChannel))*fs);% Angular acceleration is the 1st derivative of angular velocity, over it, we calcuate RMS
                for iIC = 1:size(IC,2)-1
                    AngAccRMS_PerSteps(iIC,iChannel) = rms(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
                    if iIC < size(IC,2)-1
                        AngAccRMS_PerStrides(iIC,iChannel) = rms(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
                    end
                end
                AngAccRMS_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccRMS_PerSteps(:,iChannel));
                AngAccRMS_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccRMS_PerStrides(:,iChannel));
            end
        else
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccRMS_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccRMS_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccRMS_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nan;
            end
        end
    end
    ...
    % Outputs
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        WB_SDMO(WB_index).AngAccRMS_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRMS_SigComplete,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccRMS_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRMS_AvPerSteps,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccRMS_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRMS_AvPerStrides,(NameChannels{iChannel})));
    end    
end
