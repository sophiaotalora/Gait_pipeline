function [WB_SDMO] = extract_JerkMax_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_JerkMax_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).JerkMax_SigComplete = Max of jerk (1st derivative of acceleration) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).JerkMax_AvPerSteps = average Max of jerk (1st derivative of acceleration) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).JerkMax_AvPerStrides = average Max of jerk (1st derivative of acceleration) of all stride within the WB (walking bout in the given imu data).
...
% Initialization
clear JerkMax_SigComplete
clear JerkMax_AvPerSteps
clear JerkMax_AvPerStrides
for iChannel = 1:size(NameChannels,2)
    JerkMax_SigComplete.(NameChannels{iChannel}) = [];
    JerkMax_AvPerSteps.(NameChannels{iChannel}) = [];
    JerkMax_AvPerStrides.(NameChannels{iChannel}) = [];
end
...
% Calculation of Max of Jerk - Acc
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkMax_SigComplete.(NameChannels{iChannel}) = max(diff(Acc_Detr_LPFilt(:,iChannel))*fs);% Jerk is the 1st derivative of acceleration, over it, we obtain the max
        for iIC = 1:size(IC,2)-1
            JerkMax_PerSteps(iIC,iChannel) = max(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
            if iIC < size(IC,2)-1
                JerkMax_PerStrides(iIC,iChannel) = max(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
            end
        end
        JerkMax_AvPerSteps.(NameChannels{iChannel}) = nanmean(JerkMax_PerSteps(:,iChannel));
        JerkMax_AvPerStrides.(NameChannels{iChannel}) = nanmean(JerkMax_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkMax_SigComplete.(NameChannels{iChannel}) = nan;
        JerkMax_AvPerSteps.(NameChannels{iChannel}) = nan;
        JerkMax_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.JerkMax_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(JerkMax_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.JerkMax_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(JerkMax_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.JerkMax_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(JerkMax_AvPerStrides,(NameChannels{iChannel})));
end    
%end
