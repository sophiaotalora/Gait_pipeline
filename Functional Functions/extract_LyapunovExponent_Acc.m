function [WB_SDMO] = extract_LyapunovExponent_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_LyapunovExponent_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of Lyapunov exponent - ACCELERATION
...
%% Output
% WB_SDMO(i).LyapunovRC_Acc = Local dynamic stability exponent (Rosestein method) from accelerometry data, for each signal-axis 
% WB_SDMO(i).LyapunovW_Acc = Local dynamic stability exponent (Wolf method) from accelerometry data, for each signal-axis 
...
if  StepsNumber > Settings.MinNumStrides_Complex
    ...
    for iChannel = 1:size(Acc_Detr_LPFilt,2)                
        % Rosestein
        [LyapunovRC.(NameChannels{iChannel}),~] = CalcMaxLyapConvGait(Acc_Detr_LPFilt(:,iChannel),fs,struct('J',Ly_J,'m',Ly_m,'FitWinLen',Ly_FitWinLen));
        % Wolf
        [LyapunovW.(NameChannels{iChannel}),~] = CalcMaxLyapWolfFixedEvolv(Acc_Detr_LPFilt(:,iChannel),fs,struct('J',Ly_J,'m',Ly_m));
    end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.LyapunovRC_Acc.(NameChannels{iChannel}) = nanmean(extractfield(LyapunovRC,(NameChannels{iChannel})));
    WB_SDMO.LyapunovW_Acc.(NameChannels{iChannel}) = nanmean(extractfield(LyapunovW,(NameChannels{iChannel})));
end
