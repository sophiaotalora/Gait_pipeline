function [WB_SDMO] = extract_JerkRMS_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_JerkRMS_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).JerkRMS_SigComplete = RMS of jerk (1st derivative of acceleration) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).JerkRMS_AvPerSteps = average RMS of jerk (1st derivative of acceleration) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).JerkRMS_AvPerStrides = average RMS of jerk (1st derivative of acceleration) of all stride within the WB (walking bout in the given imu data).
...
% Initialization
clear JerkRMS_SigComplete
clear JerkRMS_AvPerSteps
clear JerkRMS_AvPerStrides
for iChannel = 1:size(NameChannels,2)
    JerkRMS_SigComplete.(NameChannels{iChannel}) = [];
    JerkRMS_AvPerSteps.(NameChannels{iChannel}) = [];
    JerkRMS_AvPerStrides.(NameChannels{iChannel}) = [];
end
...
% Calculation of RMS of Jerk - Acc
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkRMS_SigComplete.(NameChannels{iChannel}) = rms(diff(Acc_Detr_LPFilt(:,iChannel))*fs);% Jerk is the 1st derivative of acceleration, over it, we calcuate RMS
        for iIC = 1:size(IC,2)-1
            JerkRMS_PerSteps(iIC,iChannel) = rms(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
            if iIC < size(IC,2)-1
                JerkRMS_PerStrides(iIC,iChannel) = rms(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
            end
        end
        JerkRMS_AvPerSteps.(NameChannels{iChannel}) = nanmean(JerkRMS_PerSteps(:,iChannel));
        JerkRMS_AvPerStrides.(NameChannels{iChannel}) = nanmean(JerkRMS_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkRMS_SigComplete.(NameChannels{iChannel}) = nan;
        JerkRMS_AvPerSteps.(NameChannels{iChannel}) = nan;
        JerkRMS_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.JerkRMS_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(JerkRMS_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.JerkRMS_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(JerkRMS_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.JerkRMS_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(JerkRMS_AvPerStrides,(NameChannels{iChannel})));
end    
%end
