function WB_SDMO = extract_StepStrideDurationMean_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_StepStrideDurationMean_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of step and stride cycles duration (mean)
...
%% Output
% WB_SDMO(i).StepDuration_Mean = mean step duration for all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).StrideDuration_Mean = mean stride duration for all strides within the WB (walking bout in the given imu data).
...
if  StepsNumber > Settings.MinNumEvents
    ...
    StepDuration_Mean = nanmean(StepDuration);
    % Stride durations
    ...
    StrideDuration_Mean = nanmean(StrideDuration);
else %not enough steps to define mean values
    StepDuration_Mean = nan;
    StrideDuration_Mean = nan;
end
%end

% Outputs
if Settings.steps== 0
    Variables= ["StepDuration","StrideDuration"];
...
% if Settings.steps== 0
%     SizeStepDuration= length(StepDuration);
%     SizeStrideDuration= length(StrideDuration);
%     WB_SDMO.First.StepDuration_Mean = StepDuration(1:3);
%     WB_SDMO.Middle.StepDuration_Mean = StepDuration((SizeStepDuration/2)-1:(SizeStepDuration/2)+1);
%     WB_SDMO.Last.StepDuration_Mean= StepDuration(end-2:end);
%     WB_SDMO.First.StrideDuration_Mean = StrideDuration(1:2);
%     WB_SDMO.Middle.StrideDuration_Mean = StrideDuration((SizeStrideDuration/2):(SizeStrideDuration/2)+1);
%     WB_SDMO.Last.StrideDuration_Mean = StrideDuration(end-1:end);
% else
    WB_SDMO.StepDuration_Mean = mean(StepDuration_Mean);
    WB_SDMO.StrideDuration_Mean = mean(StrideDuration_Mean);
end
end
