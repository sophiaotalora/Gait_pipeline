function [WB_SDMO] = extract_RMS_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_RMS_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
% Calculation of RMS - Gyro
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        RMSq_SigComplete.(NameChannels{iChannel}) = rms(Gyr_Detr_LPFilt(:,iChannel));
        for iIC = 1:size(IC,2)-1
            RMSq_PerSteps(iIC,iChannel) = rms(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel));
            if iIC < size(IC,2)-1
                RMSq_PerStrides(iIC,iChannel) = rms(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel));
            end
        end
