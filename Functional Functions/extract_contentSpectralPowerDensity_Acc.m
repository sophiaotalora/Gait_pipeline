function [WB_SDMO] = extract_contentSpectralPowerDensity_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_contentSpectralPowerDensity_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of Spectral Components - ACCELEROMETRY
...
%% Output
% WB_SDMO(i).SpectralFeatures_Acc = spectral components from acceleration signals (for each signal-axis)
...
if nargin < 5                                                               % if the settings are not predefined:
    ...
    Settings.MaxRangeFrequencyPSD = 10*fs/100;                              % maximal range of frequencies to find frequency of the peak power spectral density (PSD)
    Settings.MinRangeFrequencyPSD = 0.3*fs/100;                             % miniaml range of frequencies to find frequency of the peak power spectral density (PSD)
    Settings.HanningSize = 4;                                               % seconds
    PlotSettings.SpectralFatures = 1;                                       % activate plots to help understangind the calculation of power spectral density of signals
end
...
if StepsNumber > Settings.MinNumEvents
    ...
    %HanningWindow_Size = WB_WBD.end - WB_WBD.start;
    HanningWindow_Size= Settings.HanningSize*fs;
    Nfft = HanningWindow_Size*10;
    Signal_iD = 1;                                                         % 1 = accelerometry, 2 = gyroscope
    % get Stride Frequency
    ...
    SpectralFeatures_Acc = getFrequencyAnalysis...
        (Acc_Detr_LPFilt,Acc,fs,Nfft,Settings.N_Harm,Settings,StrideFrequency,Signal_iD,HanningWindow_Size,PlotSettings);
    FeaturesName = fieldnames(SpectralFeatures_Acc);
    FeaturesNumMax = size(FeaturesName,1);
    for iFeature = 1:FeaturesNumMax
        for iChannel = 1:size(SpectralFeatures_Acc.(FeaturesName{iFeature}),2)
            SpectralFeatures.(NameChannels{iChannel}).(VariablesPhaseNames{iFeature}) = ...
                SpectralFeatures_Acc.(FeaturesName{iFeature})(iChannel);
        end
    end
else
...
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    for iFeature = 1:size(VariablesPhaseNames,1)
        ...
        DataStraightWalk = SpectralFeatures.(NameChannels{iChannel}).(VariablesPhaseNames{iFeature}');
        %DataStraightWalk = [DataStraightWalk; DataStraightWalk_];
        %end
        WB_SDMO.SpectralFeatures_Acc.(NameChannels{iChannel}).(VariablesPhaseNames{iFeature}) = mean(DataStraightWalk);       
    end
end
end
