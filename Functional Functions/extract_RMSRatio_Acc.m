function [WB_SDMO] = extract_RMSRatio_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_RMSRatio_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of Root-mean-square of signals (ratio)  - ACCELEROMETRY
...
for iChannel = 1:size(NameChannels)-1                                   % excluding the "combined" signal
...
% Calculate RMS Ratio - Acc
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)-1
        RMSratio_SigComplete.(NameChannels{iChannel}) = rms(Acc_Detr_LPFilt(:,iChannel))./rms(Acc_Detr_LPFilt(:,4));
        for iIC = 1:size(IC,2)-1
            RMSratio_PerSteps(iIC,iChannel) = rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))./rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),4));
            if iIC < size(IC,2)-1
                RMSratio_PerStrides(iIC,iChannel) = rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))./rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),4));
            end
        end
