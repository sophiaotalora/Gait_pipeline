function data= extract_features(Fs, data, Subjects, MetaData, Settings, PlotSettings)
    for iSubject = 1%length(subjectNames)
        for iState = 1:length(MetaData.states)           
            ...
            %% SPECTRAL COMPONENTS
            % Dominant Frequency, Amplitude of PSD peak, Amplitude Normalized of PSD peak, Width, Slope, Range
            % Mean Power, Median Power, Mean Frequency, Median Frequency,Width of Normalized PSD peak
            % Slope of Normalized PSD peak, Range of Normalized PSD peak, Integrated Power,Harmonic Ratio, Index of Harmonicity
            %%%% MODIFIED SCRIPTS
            WB_SDMO=[];
            [WB_SDMO] = extract_contentSpectralPowerDensity_Acc(Imu,Fs,WB,WB_SDMO,Settings,PlotSettings);
            [WB_SDMO] = extract_contentSpectralPowerDensity_Gyro(Imu,Fs,WB,WB_SDMO,Settings,PlotSettings);
            [WB_SDMO] = extract_HarmonicRatio_PerStrides_Acc(Imu,Fs,WB,WB_SDMO,Settings,PlotSettings);
            [WB_SDMO] = extract_HarmonicRatio_PerStrides_Gyro(Imu,Fs,WB,WB_SDMO,Settings,PlotSettings);
                %{
                % Pace
                try
