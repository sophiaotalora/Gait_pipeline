function [WB_SDMO] = extract_AngularAccMin_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_AngularAccMin_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).AngAccMin_SigComplete = Min of angular acceleration (1st derivative of angular velocity) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).AngAccMin_AvPerSteps = average Min of angular acceleration (1st derivative of angular velocity) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).AngAccMin_AvPerStrides = average Min of angular acceleration (1st derivative of angular velocity) of all stride within the WB (walking bout in the given imu data).
...
for WB_index = 1:WB_Number                                                  % loop over each WB
    ...
    % Initialization
    clear AngAccMin_SigComplete
    clear AngAccMin_AvPerSteps
    clear AngAccMin_AvPerStrides
    for iChannel = 1:size(NameChannels,2)
        AngAccMin_SigComplete.(NameChannels{iChannel}) = [];
        AngAccMin_AvPerSteps.(NameChannels{iChannel}) = [];
        AngAccMin_AvPerStrides.(NameChannels{iChannel}) = [];
    end
    for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
        ...
        % Calculation of Min of Angular Acceleration - Gyro
        if StepsNumber > Settings.MinNumEvents
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccMin_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = min(diff(Gyr_Detr_LPFilt(:,iChannel))*fs);% Angular acceleration is the 1st derivative of angular velocity, over it, we calcuate min
                for iIC = 1:size(IC,2)-1
                    AngAccMin_PerSteps(iIC,iChannel) = min(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
                    if iIC < size(IC,2)-1
                        AngAccMin_PerStrides(iIC,iChannel) = min(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
                    end
                end
                AngAccMin_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccMin_PerSteps(:,iChannel));
                AngAccMin_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccMin_PerStrides(:,iChannel));
            end
        else
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccMin_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccMin_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccMin_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nan;
            end
        end
    end
    ...
    % Outputs
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        WB_SDMO(WB_index).AngAccMin_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(AngAccMin_SigComplete,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccMin_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(AngAccMin_AvPerSteps,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccMin_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(AngAccMin_AvPerStrides,(NameChannels{iChannel})));
    end    
end
