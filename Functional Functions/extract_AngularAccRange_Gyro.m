function [WB_SDMO] = extract_AngularAccRange_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_AngularAccRange_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).AngAccRange_SigComplete = Range of angular acceleration (1st derivative of angular velocity) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).AngAccRange_AvPerSteps = average Range of angular acceleration (1st derivative of angular velocity) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).AngAccRange_AvPerStrides = average Range of angular acceleration (1st derivative of angular velocity) of all stride within the WB (walking bout in the given imu data).
...
for WB_index = 1:WB_Number                                                  % loop over each WB
    ...
    % Initialization
    clear AngAccRange_SigComplete
    clear AngAccRange_AvPerSteps
    clear AngAccRange_AvPerStrides
    for iChannel = 1:size(NameChannels,2)
        AngAccRange_SigComplete.(NameChannels{iChannel}) = [];
        AngAccRange_AvPerSteps.(NameChannels{iChannel}) = [];
        AngAccRange_AvPerStrides.(NameChannels{iChannel}) = [];
    end
    for StraightWalk_Index = 1:StraightWalk_Number                          % loop over straight walks within each WB
        ...
        % Calculation of Ranfe of Angular Acceleration - Gyro
        if StepsNumber > Settings.MinNumEvents
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccRange_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = max(diff(Gyr_Detr_LPFilt(:,iChannel))*fs)-min(diff(Gyr_Detr_LPFilt(:,iChannel))*fs);% Angular acceleration is the 1st derivative of angular velocity, over it, we calcuate max
                for iIC = 1:size(IC,2)-1
                    AngAccRange_PerSteps(iIC,iChannel) = max(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs)-min(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
                    if iIC < size(IC,2)-1
                        AngAccRange_PerStrides(iIC,iChannel) = max(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs)-min(diff(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
                    end
                end
                AngAccRange_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccRange_PerSteps(:,iChannel));
                AngAccRange_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nanmean(AngAccRange_PerStrides(:,iChannel));
            end
        else
            for iChannel = 1:size(Gyr_Detr_LPFilt,2)
                AngAccRange_SigComplete(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccRange_AvPerSteps(StraightWalk_Index).(NameChannels{iChannel}) = nan;
                AngAccRange_AvPerStrides(StraightWalk_Index).(NameChannels{iChannel}) = nan;
            end
        end
    end
    ...
    % Outputs
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)
        WB_SDMO(WB_index).AngAccRange_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRange_SigComplete,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccRange_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRange_AvPerSteps,(NameChannels{iChannel})));
        WB_SDMO(WB_index).AngAccRange_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(AngAccRange_AvPerStrides,(NameChannels{iChannel})));
    end    
end
