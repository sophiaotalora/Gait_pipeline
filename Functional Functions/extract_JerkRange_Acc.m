function [WB_SDMO] = extract_JerkRange_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_JerkRange_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
...
%% Output
% WB_SDMO(i).JerkRange_SigComplete = Range of jerk (1st derivative of acceleration) of complete signal (walking bout in the given imu data).
% WB_SDMO(i).JerkRange_AvPerSteps = average Range of jerk (1st derivative of acceleration) of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).JerkRange_AvPerStrides = average Range of jerk (1st derivative of acceleration) of all stride within the WB (walking bout in the given imu data).
...
% Initialization
clear JerkRange_SigComplete
clear JerkRange_AvPerSteps
clear JerkRange_AvPerStrides
for iChannel = 1:size(NameChannels,2)
    JerkRange_SigComplete.(NameChannels{iChannel}) = [];
    JerkRange_AvPerSteps.(NameChannels{iChannel}) = [];
    JerkRange_AvPerStrides.(NameChannels{iChannel}) = [];
end
...
% Calculation of Range of Jerk - Acc
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkRange_SigComplete.(NameChannels{iChannel}) = max(diff(Acc_Detr_LPFilt(:,iChannel))*fs)-min(diff(Acc_Detr_LPFilt(:,iChannel))*fs);% Jerk is the 1st derivative of acceleration, over it, we obtain the range
        for iIC = 1:size(IC,2)-1
            JerkRange_PerSteps(iIC,iChannel) = max(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs)-min(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel))*fs);
            if iIC < size(IC,2)-1
                JerkRange_PerStrides(iIC,iChannel) = max(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs)-min(diff(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel))*fs);
            end
        end
        JerkRange_AvPerSteps.(NameChannels{iChannel}) = nanmean(JerkRange_PerSteps(:,iChannel));
        JerkRange_AvPerStrides.(NameChannels{iChannel}) = nanmean(JerkRange_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        JerkRange_SigComplete.(NameChannels{iChannel}) = nan;
        JerkRange_AvPerSteps.(NameChannels{iChannel}) = nan;
        JerkRange_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.JerkRange_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(JerkRange_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.JerkRange_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(JerkRange_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.JerkRange_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(JerkRange_AvPerStrides,(NameChannels{iChannel})));
end    
%end
