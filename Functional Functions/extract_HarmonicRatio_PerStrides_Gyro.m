function [WB_SDMO] = extract_HarmonicRatio_PerStrides_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_HarmonicRatio_PerStrides_Gyro(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of HARMONIC RATIO PER STRIDES - ANGULAR VELOCITY / GYROSCOPE
...
for iChannel = 1:size(NameChannels,2)-1                                 % excluding resultant Gyr
...
% Calculation of Harmonic Ratio - Gyr
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Gyr_Detr_LPFilt,2)-1                      % excluding resultant Gyr
        for iIC = 1:size(IC,2)-2
            if iChannel == 2 %in case of gyroscope data, the harmonicity is as for ML in Acc
                HarmonicRatio_PerStrides(iIC,iChannel) = HRgenerateAPV(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel)); % in the case of gyroscope, harmonicity is opposite than for accelerometry (around VT and AP axes, the periodicity is in stride cycles)
            else
                HarmonicRatio_PerStrides(iIC,iChannel) = HRgenerateML(Gyr_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel));
            end
        end
...
% Outputs
for iChannel = 1:size(Gyr_Detr_LPFilt,2)-1                              % excluding resultant Gyr
    WB_SDMO.HarmonicRatio_AvPerStrides_Gyr.(NameChannels{iChannel}) = mean(extractfield(HarmonicRatio_AvPerStrides,(NameChannels{iChannel}))); % mean for the 4 straight walk episodes
end    
%end
