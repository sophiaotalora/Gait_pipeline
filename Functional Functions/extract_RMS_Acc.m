function [WB_SDMO] = extract_RMS_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)
%[WB_SDMO] = extract_RMS_Acc(imu,fs,WB_WBD,WB_SDMO,Settings,PlotSettings)

%% Calculation of Root-mean-square of signals (pure) - ACCELEROMETRY
...
%% Output
% WB_SDMO(i).RMSq_SigComplete = RMS of complete signal (walking bout in the given imu data).
% WB_SDMO(i).RMSq_AvPerSteps = average RMS of all steps within the WB (walking bout in the given imu data).
% WB_SDMO(i).RMSq_AvPerStrides = average RMS of all stride within the WB (walking bout in the given imu data).
...
% Initialization
clear RMSq_SigComplete
clear RMSq_AvPerSteps
clear RMSq_AvPerStrides
for iChannel = 1:size(NameChannels)
    RMSq_SigComplete.(NameChannels{iChannel}) = [];
    RMSq_AvPerSteps.(NameChannels{iChannel}) = [];
    RMSq_AvPerStrides.(NameChannels{iChannel}) = [];
end
...
if StepsNumber > Settings.MinNumEvents
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        RMSq_SigComplete.(NameChannels{iChannel}) = rms(Acc_Detr_LPFilt(:,iChannel));
        for iIC = 1:size(IC,2)-1
            RMSq_PerSteps(iIC,iChannel) = rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+1),iChannel));
            if iIC < size(IC,2)-1
                RMSq_PerStrides(iIC,iChannel) = rms(Acc_Detr_LPFilt(IC(iIC):IC(iIC+2),iChannel));
            end
        end
        RMSq_AvPerSteps.(NameChannels{iChannel}) = nanmean(RMSq_PerSteps(:,iChannel));
        RMSq_AvPerStrides.(NameChannels{iChannel}) = nanmean(RMSq_PerStrides(:,iChannel));
    end
else
    for iChannel = 1:size(Acc_Detr_LPFilt,2)
        RMSq_SigComplete.(NameChannels{iChannel}) = nan;
        RMSq_AvPerSteps.(NameChannels{iChannel}) = nan;
        RMSq_AvPerStrides.(NameChannels{iChannel}) = nan;
    end
end
%end
...
% Outputs
for iChannel = 1:size(Acc_Detr_LPFilt,2)
    WB_SDMO.RMSq_SigComplete.(NameChannels{iChannel}) = nanmean(extractfield(RMSq_SigComplete,(NameChannels{iChannel})));
    %WB_SDMO.RMSq_AvPerSteps.(NameChannels{iChannel}) = nanmean(extractfield(RMSq_AvPerSteps,(NameChannels{iChannel})));
    %WB_SDMO.RMSq_AvPerStrides.(NameChannels{iChannel}) = nanmean(extractfield(RMSq_AvPerStrides,(NameChannels{iChannel})));
end    
%end
