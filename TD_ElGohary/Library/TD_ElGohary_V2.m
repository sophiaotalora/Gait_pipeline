...
if isstruct(gs) && ~isempty(fieldnames(gs))
    ...
    for k = 1:length(gs)
        ...
        gs_start_sample = round(current_gs.GaitSequence_Start*fs)+1; % add 1 to avoid use of sample 0 which is not allowed in Matlab
        gs_end_sample = min(floor(current_gs.GaitSequence_End*fs), length(imu.LowerBack.acc)); 

        % change units
        if Gyr_units == 1
            imu.LowerBack.gyr = (imu.LowerBack.gyr)*(180/pi);
        end
