...
AllFieldsT=GetFields(Data,'data',{});
AllFields=AllFieldsT(find(contains(AllFieldsT,'Acc')));             %Searching for the Acc information
AllFields2=AllFieldsT(find(contains(AllFieldsT,'Timestamp')));
...
for Trials_list_Index = 1:length(Trials_list)
    ...
    %Fields_split2 = strsplit(AllFields2{Trials_list_Index}, '.');
    imu=getfield(Data,Fields_split{2:end-1});                               %imu contains under Lowerback the fields: Acc,Gyr and Fs
    ...
    %timestamp_field = getfield(data, Fields_split2{2:end});
    %GaitSequence_End = timestamp_field(end);
    %gsd_result = struct('GaitSequence_Start',[1],'GaitSequence_End',GaitSequence_End,'GSD_fs',[128]);
